document.addEventListener('DOMContentLoaded', () => {
  ...
  const productCount = data.pods.length;
  ...
  data.pods.forEach((pod, index) => {
    ...
    // Escape single quotes in the JSON string for data-pod attribute
  ...
  document.querySelectorAll('.buy-button').forEach(button => {
    button.addEventListener('click', (e) => {
      try {
        // Unescape on parsing if previously escaped with &apos;
  ...
  if (closeModal) {
    closeModal.addEventListener('click', () => {
      if (modal) {
        modal.classList.remove('show-modal'); // Remove class to hide modal
      } else {
  ...
  if (addressForm) {
    addressForm.addEventListener('submit', async (e) => {
      ...
      try {
        ...
        if (modal) {
          modal.classList.remove('show-modal'); // Hide modal after submission
        }
        addressForm.reset();
      } catch (error) {
